...
// function to fetch current weather in Malmö 
const fetchWeather = () => {
    fetch(API)
...
// Function to fetch data from json (for experience section)
...
const renderExperiences = (experiences) => {
    ...
    container.innerHTML = experiences.map(exp => `
        <div class="exp-card">
            <div class="year-icon-wrapper">
                <img class="icon-exp" src="${exp.icon}" alt="${exp.iconAlt}"/>
                <h5>${exp.period}</h5>
            </div>
    ...
    const buttons = container.querySelectorAll(".btn-show-more");

    buttons.forEach(button => {
        button.addEventListener("click", () => {
            const desc = button.nextElementSibling;
            desc.classList.toggle("show");
            button.textContent = desc.classList.contains("show") ? "SHOW LESS" : "SHOW MORE";
        });
    });
};
